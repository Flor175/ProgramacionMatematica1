...
class Ventana_Registro(QtWidgets.QWidget, Ui_Registro): 
        ...
        def registro_usuarios(self):
            ...
            output = open("proyecto.pas", "rb")
            registrados = pickle.load(output)
            output.close()
            for key in registrados:
                value = registrados.get(correo)
                if key == correo and contraseña == value:
                    self.dialogo.label.setText("Usuario ya existente")
                    self.dialogo.show()
                    return False 
                else:
                    output = open("proyecto.pas", "rb")
                    registrados1 = pickle.load(output)
                    output.close()
                    print(registrados1)
                    registrados1.update({correo:contraseña})
            ...
            if self.validar_correo(email) == True and self.validar_password() == True and self.confirmar() == True:
                ...
                for key in registrados:
                    ...
                    if key == correo and contraseña == value:
                        ...
                        pickle.dump(registrados1, archivo)
                        archivo.close()
                        Enviar_correo.enviar_correo(email)
                        self.dialogo.label.setText("Usuario creado")
                        self.dialogo.show()
                        self.close()
            elif email == "":
...
class Ventana_Login(QtWidgets.QWidget, Ui_Login): 
        ...
        def validar_usuario(self):
            ...
            output.close()
            print(registrados1)
            for key in registrados1:
                value = registrados1.get(correo)
                if key == correo and contraseña == value:
                    Verificar_internet.internet()
                    self.close()
